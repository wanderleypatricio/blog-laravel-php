...
class CreatePapelsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('papels_permissao', function (Blueprint $table) {
            $table->integer('permissao_id')->unsigned();
            $table->integer('papel_id')->unsigned();
            $table->foreign('permissao_id')->references('id')->on('permissaos')->onDelete('cascade');
            $table->foreign('papel_id')->references()->on('id')->on('papels')->onDelete('cascade');
            $table->primary(['permissao_id','papel_id']);
        });
        Schema::create('papels_user', function (Blueprint $table) {
